...
function admin_files_route($action = '', $params = []) {
    ...
    ?>
    <style>
        .fm-wrap { display:flex; gap:20px; align-items:flex-start; flex-wrap:wrap; }
        .fm-tree { flex:1 1 320px; min-width:350px; background:#fff; border:1px solid var(--rang-border,#dde1e6); border-radius:12px; padding:14px; }
        ...
        .fm-detail { flex:2 1 420px; min-width:360px; background:#fff; border:1px solid var(--rang-border,#dde1e6); border-radius:12px; padding:14px; }
    ...
    <div class="fm-wrap">
        ...
        <div class="fm-tree">
            ...
            <form method="post" action="<?= BASE_URL ?>mod/files/mkdir" class="fm-form">
                ...
                <button type="submit" class="btn-sm">+ ساخت</button>
            </form>
        </div>

        <!-- ستون چپ: جزئیات + آپلود -->
        <div class="fm-detail">
            <h4><i class="fa-solid fa-file"></i> جزئیات و آپلود</h4>
    ...
    <div class="fm-card fm-form">
        <form method="post" action="<?= BASE_URL ?>mod/files/upload" enctype="multipart/form-data">
            ...
            <?php else: ?>
                <p style="color:var(--rang-gray,#999);">برای مشاهده جزئیات و محل استفاده، یک فایل را از درخت سمت راست انتخاب کنید.</p>
            <?php endif; ?>
        </div>
    </div>
    <?php
...
function public_upload_route() {
    ...
    if (!$enabled) {
        include __DIR__ . '/../../ghaleb/mehrsam/sarsafhe.php';
